...
def validate(circles, blockers):
        ...
        for circle in circles:
            for block in blockers:
                ...
                if math.sqrt((x - bx)**2 + (y - by)**2) < r:
                    print('There is blocker in the circles')
                    return False
    ...
    for circle1 in circles:
        for circle2 in circles:
            ...
            y2 = circle2[1]
            r2 = circle2[2]
            if math.sqrt((x1 - x2)**2 + (y1 - y2)**2) < (r1 + r2) and (x1 != x2 or y1 != y2):
                print('There are some circles covering others')
                return False
